...
function enforceHold(account, label = 'Your account') {
  if (account.isBlocked) {
    const err = new Error(
      `${label} (...${account.accountNumber}) has been placed on hold: "${account.blockReason || 'Security review'}". Please contact customer service at support@redwoodcrestinc.com for assistance.`
    );
    err.status = 403;
